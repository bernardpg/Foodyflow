//
//  ShoppingListBtnPanelView.swift
//  Foodyflow
//
//  Created by 曹珮綺 on 7/16/22.
//
...
protocol ShopButtonPanelDelegate: NSObject {
...
class ShoppingListBtnPanelView: UIView {

  weak var delegate: ShopButtonPanelDelegate?
  ...
  lazy var dogButton: UIButton = {
    let button = UIButton(frame: .zero)
    button.setImage(UIImage(named: "carts")?.resize(to: .init(width: 32, height: 32)), for: .normal)
      button.tag = 2
//    button.setTitle("🛒", for: .normal)
    button.layer.cornerRadius = buttonSize / 2
...
extension ShoppingListBtnPanelView {
